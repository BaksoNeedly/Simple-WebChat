...
class HTTPResponse:
    ...
    def get_response(self) -> str:
        self.build().decode(config.FORMAT)

    def build(self) -> bytes:
        response = self._response.encode(config.FORMAT)
        ...
        if isinstance(self._body, str):
            response += self._body.encode(config.FORMAT)
        elif isinstance(self._body, bytes):
            response += self._body
        else:
            response += str(self._body).encode(config.FORMAT)
